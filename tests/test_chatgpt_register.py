...
class RegistrationEngineFlowTests(unittest.TestCase):
    ...
    def test_run_stops_after_new_registration_without_token_exchange(self):
        engine = self._make_engine()
    ...
    def test_run_does_not_login_or_take_token_for_existing_account(self):
        engine = self._make_engine()
        ...
        self.assertFalse(result.success)
        self.assertIn("不再执行登录或 OAuth 取 token", result.error_message)
